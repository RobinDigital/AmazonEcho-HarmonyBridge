...
namespace HarmonyHub
{
    class HarmonyDocuments
    {
        ...
        public static HarmonyDocument EndActivityDocument(string activityId)
        {
            var document = new HarmonyDocument();

            var element = new Element("oa");
            element.Attributes.Add("xmlns", "connect.logitech.com");
            element.Attributes.Add("mime", "vnd.logitech.harmony/vnd.logitech.harmony.activityengine?runactivity");
            //element.Value = "activityId="+activityId+":timestamp=0:args={\"rule\":\"end\"}";
            element.Value = "{\"activityId\":\"" + activityId + "\",\"timestamp\":\"0\",\"async\":true,\"args\":{\"rule\":\"end\"}}";
            document.AddChild(element);
            return document;
        }
